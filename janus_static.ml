...
module Plugin = struct
  ...
  let create_offer (plugin:t) media trickle =
    prepare_offer_or_answer media trickle |> call_js_method (fun x -> plugin##createAnswer x)

  let handle_remote_jsep (plugin:t) jsep =
    call_js_method (fun x -> plugin##handleRemoteJsep x) [| ("jsep", Js.Unsafe.inject jsep) |]
  ...
  let dtmf (plugin:t) tones duration gap =
    ...
    call_js_method (fun x -> plugin##dtmf x) [| tones'; duration'; gap' |]

  let data (plugin:t) text =
    let text' = ("text", Js.Unsafe.inject @@ Js.string text) in
    call_js_method (fun x -> plugin##data x) [| text' |]

  let get_bitrate (plugin:t) = plugin##getBitrate () |> Js.to_string

  let hangup (plugin:t) send_request = plugin##hangup (Js.bool send_request)

  let detach (plugin:t) = call_js_method (fun x -> plugin##detach x) [| |]
...
module Session = struct
  ...
  let is_connected (session:t) = session##isConnected () |> Js.to_bool
